...
export class RegistroPage {
  ...
  async escanearDni() {
    ...
    try {
      ...
      if (!supported) {
        this.errorEscaneo.set(
          'Este dispositivo no soporta la lectura de códigos QR.',
        );
        return;
      }
      ...
      if (
        e?.message === 'scan canceled.' ||
        e?.errorMessage === 'scan canceled.'
      )
        return;
      if (e?.isAcquireTimeout) {
        this.errorEscaneo.set('Tiempo de espera agotado. Intentá de nuevo.');
      } else {
        this.errorEscaneo.set(
          'Ocurrió un error al leer el DNI. Intentá de nuevo.',
        );
      }
    }
  }
  ...
  async validar(): Promise<boolean> {
    ...
    else if (!/^\d{7,8}$/.test(f.dni))
      errs['dni'] = 'El DNI debe tener 7 u 8 dígitos numéricos.';
    if (!f.cuil.trim()) errs['cuil'] = 'El CUIL es obligatorio.';
    else if (!/^\d{11}$/.test(f.cuil))
      errs['cuil'] = 'El CUIL debe tener 11 dígitos numéricos.';
    if (!f.email.trim())
    ...
    if (!f.confirmarPassword.trim())
      errs['confirmarPassword'] = 'Debe confirmar la contraseña.';
  ...
  async registrar() {
    ...
    try {
      ...
      await this.haptics.error();
      this.errorGeneral.set(
        (e as Error).message || 'Ocurrió un error al registrarse.',
      );
    } finally {
